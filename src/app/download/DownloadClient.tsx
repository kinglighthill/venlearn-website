...
export default function DownloadClient() {
  return (
    <div className="overflow-hidden bg-white px-5 pb-24 pt-32 sm:px-8 lg:px-10">
      <div className="absolute inset-x-0 top-0 -z-10 h-[38rem] bg-[radial-gradient(circle_at_14%_16%,rgba(38,97,172,0.14),transparent_30%),radial-gradient(circle_at_84%_18%,rgba(38,97,172,0.08),transparent_28%),linear-gradient(180deg,#f3f7fc_0%,#ffffff_82%)]" />
      <div className="mx-auto max-w-7xl">
        <div className="mx-auto max-w-5xl text-center">
          <p className="text-sm font-black uppercase tracking-[0.24em] text-[#2661ac]">No old downloads needed</p>
          ...
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link href="/book-demo" className="flex min-h-14 items-center justify-center gap-2 rounded-full bg-[#101828] px-8 py-4 font-black text-white">
              See the workspace
              <ArrowRight className="h-5 w-5" />
            </Link>
            <Link href="/book-demo" className="flex min-h-14 items-center justify-center rounded-full border border-[#eaecf0] bg-white px-8 py-4 font-black text-[#101828] shadow-sm">
        ...
        <div className="mt-14 grid gap-5 lg:grid-cols-4">
          {accessCards.map(([title, description, Icon]) => (
            <article key={title as string} className="rounded-[1.75rem] border border-[#eaecf0] bg-white p-6 shadow-xl shadow-[#101828]/5">
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#2661ac] text-white">
        ...
        <section className="mt-16 rounded-[2rem] bg-[#101828] p-6 text-white sm:p-10">
          <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
            <div>
              <p className="text-sm font-black uppercase tracking-[0.24em] text-[#6f9fd3]">Implementation included</p>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {["Data import", "Role setup", "Teacher training", "Parent launch", "Finance configuration", "Leadership reporting"].map((item) => (
                <div key={item} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/8 p-4">
                  <Check className="h-5 w-5 text-[#2661ac]" />
