      ...
      <div className="flex overflow-x-auto gap-6 mb-24 py-8 snap-x snap-mandatory -mx-4 px-4 md:mx-0 md:px-0 scrollbar-hide [&::-webkit-scrollbar]:hidden [-ms-overflow-style:'none'] [scrollbar-width:'none']">
        {plans.map((plan, index) => (
          <motion.div
            ...
            className={`relative rounded-3xl p-6 border flex flex-col transition-all duration-300 min-w-[300px] md:min-w-[320px] snap-center ${
              plan.highlight 
          ...
          <table className="w-full min-w-[800px]">
            <thead>
              <tr className="bg-white">
                ...
                {plans.map(plan => (
                  <th key={plan.name} className={`p-6 text-center font-bold ${
                    plan.highlight ? "text-brand-primary" : "text-slate-700" 
                  }`}>
                    {plan.name}
                  </th>
                ))}
